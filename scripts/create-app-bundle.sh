...
# Build if needed
if [ ! -x "$BUILD_DIR/Preferences" ]; then
  echo "Building Preferences..."
  (cd "$REPO_DIR" && swift build -c release --target Preferences)
fi
if [ ! -x "$BUILD_DIR/MouseFix" ]; then
  echo "Building MouseFix..."
  (cd "$REPO_DIR" && swift build -c release --target MouseFix)
fi
